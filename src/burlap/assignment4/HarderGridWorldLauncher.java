...
import burlap.assignment4.util.SalesmenRewardFunction;
...
public class HarderGridWorldLauncher {
...
//	private static Integer mapLen = map.length-1;
	...
	public static void main(String[] args) {
		...
		int bhX = 10;
		int bhY = map[0].length-1-5;
		// 

		BasicGridWorld gen = new BasicGridWorld(map,maxX,maxY); //0 index map is 18X10
		Domain domain = gen.generateDomain();

		State initialState = BasicGridWorld.getExampleState(domain, maxX -1, maxY);

//		RewardFunction rf = new BasicRewardFunction(maxX,maxY); //Goal is at the top right grid
		RewardFunction rf = new SalesmenRewardFunction(maxX,maxY,bhX,bhY); 
